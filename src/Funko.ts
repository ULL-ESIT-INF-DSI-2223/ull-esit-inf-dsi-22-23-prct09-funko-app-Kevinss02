...
export class Funko implements IFunkoData {
  ...
  constructor(
    ...
    if (franchiseNumber < 0 || !(Number.isInteger(franchiseNumber))) { throw new Error(`Invalid Franchise Number: ${franchiseNumber}`) }
    if (marketValue < 0 || !isFinite(marketValue)) { throw new Error(`Invalid Market Value: ${marketValue}`); }
  }
